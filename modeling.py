...
class Encoder(nn.Module):
    ...
    def forward(self, hidden_states, struc_tokens, input_ids, temp_w):
        attn_weights = []
        hid_ori = []
        structure = []
        ...
        for i, layer in enumerate(self.layer):
            ...
            if i == 4:
                ...
                temp_weight4 = weights
                if temp_w == 8:#等于8时进行裁剪图像输出，等于9时验证集只进行单路输入验证，等于10时不进行空间信息学习融合
                    img_crop, img_drop = attention_crop_drop(temp_weight4, input_ids)
            elif i > 8 and temp_w != 10:#空间信息学习
        ...
        if temp_w == 8:
            return part_encoded, hid_ori, img_crop, img_drop
        else:
...
class Transformer(nn.Module):
    ...
    def forward(self, input_ids, temp_w):
        embedding_output, struc_tokens = self.embeddings(input_ids)

        if temp_w == 8:
            part_encoded, hid, img_crop, img_drop = self.encoder(embedding_output, struc_tokens, input_ids, temp_w)
            return part_encoded, hid, img_crop, img_drop
        else:
            part_encoded, hid = self.encoder(embedding_output, struc_tokens, input_ids, temp_w)
            return part_encoded, hid


class VisionTransformer(nn.Module):
    def __init__(self, config, img_size=224, num_classes=21843, smoothing_value=0, zero_head=False, temp_w=None):
    ...
    def forward(self, x, labels=None, step=0, global_step=10000, temp_w=9):
        #print(temp_w)
        if temp_w == 8:
            part_tokens, hid, img_crop, img_drop = self.transformer(x, temp_w)
        else:
            part_tokens, hid = self.transformer(x, temp_w)

        if temp_w == 8:
            final_hid = torch.cat((hid[-2], hid[-1], part_tokens[:, 0]), dim=-1)#空间特征融合
            part_logits = self.part_head(final_hid)
        elif temp_w == 9:
            final_hid = torch.cat((hid[-2], hid[-1], part_tokens[:, 0]), dim=-1)
            part_logits = self.part_head(final_hid)
        else:
            part_logits = self.head(part_tokens[:, 0])
        if labels is not None:
            ...
            if temp_w == 8:
                return loss, part_logits, img_crop, img_drop
            else:
                return loss, part_logits
        else:
            #print(temp_w)
            if temp_w == 8:
                return part_logits, img_crop, img_drop
            else:
                return part_logits
